...
class Shotgun(defaultWeapon.Weapon):
        ...
        if mousestate[0] and self.ammo != 0:
            if self.delay == 0:
                if self.mouseswith['0']:
                    self.ammo -= 1
                    bulletClass.buckshotBullet.Buckshot(shooter=self.shooter, targets=self.targets, mouse=mouse, spreadangle=self.spreadangle, shotrange=self.shotrange, damage=self.damage)
