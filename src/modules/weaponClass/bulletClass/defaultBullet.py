...
class Bullet(entityClass.defaultEntity.Entity):
    ...
    def __init__(self, shooter, targets, mouse, spreadangle=0, shotrange=0, damage=0, layer=1):
        super().__init__()
        ...
        self.firstplace =  shooter.rect.center

        #spread
        angle = random.randint(-spreadangle//2, spreadangle//2+1)
        rad_alfa = angle * (math.pi/180)

        dxvector = mouse[0] - shooter.rect.center[0]
        dyvector = mouse[1] - shooter.rect.center[1]
